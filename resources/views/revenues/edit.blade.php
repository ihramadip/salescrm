                    ...
                    <form action="{{ route('revenues.update', $revenue) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="deal_id" name="deal_id" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-emerald-500 dark:focus:border-emerald-600 focus:ring-emerald-500 dark:focus:ring-emerald-600 rounded-md shadow-sm">
                                    ...
                                    @foreach($deals as $deal)
                                        <option value="{{ $deal->id }}" {{ old('deal_id', $revenue->deal_id) == $deal->id ? 'selected' : '' }}>{{ $deal->title }} ({{ number_format($deal->value, 2) }})</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="amount" name="amount" type="number" step="0.01" class="mt-1 block w-full" :value="old('amount', $revenue->amount)" required />
                                <x-input-error class="mt-2" :messages="$errors->get('amount')" />
                            </div>
                            
                            <div>
                                <x-input-label for="revenue_date" :value="__('Revenue Date')" />
                                <x-text-input id="revenue_date" name="revenue_date" type="date" class="mt-1 block w-full" :value="old('revenue_date', $revenue->revenue_date)" required />
